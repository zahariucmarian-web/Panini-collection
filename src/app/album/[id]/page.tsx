...
export default function AlbumTracker({ params }: { params: { id: string } }) {
  ...
  // State for sticker collection: { "USA 1": count, "CAN 5": count, ... }
  ...
  // Debounced auto-save mechanism to avoid hammering Vercel KV database
  ...
  // Helper to handle sticker interaction
  const handleStickerClick = (stickerKey: string) => {
    setStickers(prev => {
      ...
      if (current === 0) {
        ...
        delete nextStickers[stickerKey]; // Toggling off removes key or sets to 0
      }
      return nextStickers;
    });
  };
  ...
  const incrementDuplicate = (stickerKey: string, e: React.MouseEvent) => {
    e.stopPropagation(); // Avoid triggering main card click
    setStickers(prev => {
      const current = prev[stickerKey] || 0;
      return {
        ...prev,
  ...
  // Helper to count visible stickers in a country based on current search & filter
  ...
  if (loading) {
    return (
      <div className="flex-1 flex flex-col items-center justify-center min-h-screen bg-[#f7f5ee]">
        <div className="flex flex-col items-center space-y-4">
          <RefreshCw className="w-12 h-12 text-[#8a1538] animate-spin" />
          <h2 className="text-lg font-semibold text-slate-700">Se încarcă albumul tău din cloud...</h2>
  ...
  return (
    <div className="flex-1 flex flex-col bg-[#f7f5ee] min-h-screen pb-24">
        ...
        <div className="max-w-6xl mx-auto px-4 py-3 flex items-center justify-between">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <div className="flex items-center space-x-2">
                <span className="font-bold text-slate-800 tracking-tight text-sm md:text-base">Album Panini 2026</span>
                <span className="bg-[#8a1538] text-white text-xs px-2 py-0.5 rounded font-black uppercase tracking-wider">
                  {id}
                </span>
              </div>
              <div className="flex items-center space-x-1.5 mt-0.5">
                {saveStatus === "se_salveaza" && (
                  <>
                    <RefreshCw className="w-3 h-3 text-[#b49141] animate-spin" />
                    <span className="text-[10px] text-[#b49141] font-medium">Se salvează în Cloud...</span>
                  </>
                )}
                {saveStatus === "salvat" && (
                  <>
                    <div className="w-1.5 h-1.5 bg-emerald-500 rounded-full"></div>
                    <span className="text-[10px] text-emerald-500 font-semibold">Salvat în cloud</span>
                  </>
                )}
                {saveStatus === "eroare" && (
                  <>
                    <div className="w-1.5 h-1.5 bg-rose-500 rounded-full"></div>
                    <span className="text-[10px] text-rose-500 font-bold">Eroare conexiune (Offline fallback)</span>
                  </>
                )}
              </div>
            </div>
          ...
          <div className="flex items-center space-x-2">
            <button
              ...
              className="flex items-center space-x-1.5 px-3 py-1.5 bg-[#f7f5ee] hover:bg-[#eae8df] text-[#8a1538] text-xs font-semibold rounded-lg transition"
            >
              {linkCopied ? (
                <>
      ...
      <main className="max-w-6xl w-full mx-auto px-4 mt-6 flex-1 flex flex-col md:flex-row md:space-x-6 space-y-6 md:space-y-0">
        ...
        <div className="w-full md:w-80 flex-shrink-0 space-y-6">
          ...
          {/* Card: Progres General */}
          <div className="bg-white rounded-2xl p-6 border border-slate-100 shadow-sm relative overflow-hidden">
            <div className="absolute top-0 right-0 w-24 h-24 bg-[#b49141] opacity-5 rounded-full blur-xl transform translate-x-6 -translate-y-6"></div>
            
            <h3 className="text-sm font-semibold text-slate-400 uppercase tracking-wider mb-4">Progres General</h3>
            ...
            <div className="flex items-center space-x-4">
              <div className="relative flex-shrink-0 flex items-center justify-center w-20 h-20 rounded-full border-4 border-slate-100" style={{
                background: `conic-gradient(#8a1538 ${completionPercentage}%, #f1f5f9 0)`
              }}>
                {/* Center cutout */}
              ...
              <div className="flex-1 space-y-1">
                ...
                <div className="text-xs text-slate-400 font-medium">din {TOTAL_STICKERS_COUNT} de stickere</div>
              </div>
            </div>

            {/* Quick Progress Bar */}
            <div className="w-full bg-slate-100 rounded-full h-2 mt-5">
              <div className="bg-[#8a1538] h-2 rounded-full transition-all duration-300" style={{ width: `${completionPercentage}%` }}></div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4 mt-6 pt-6 border-t border-slate-100 text-center">
              <div>
                ...
                <div className="text-[10px] text-slate-400 uppercase font-semibold">Mai ai nevoie</div>
              </div>
              <div>
                <div className="text-sm font-bold text-amber-600">{totalDuplicatesCount}</div>
                <div className="text-[10px] text-slate-400 uppercase font-semibold">Dubluri</div>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-2xl p-2 border border-slate-100 shadow-sm flex flex-col space-y-1">
            <button
              ...
              className={`flex items-center space-x-3 px-4 py-3 rounded-xl text-sm font-bold transition ${
                activeTab === "album" 
                  ? "bg-[#8a1538] text-white shadow-md shadow-[#8a1538]/10" 
        ...
        <div className="flex-1 flex flex-col space-y-6">
          ...
          {activeTab === "album" && (
            <>
              {/* Header Toolbar: Search & Quick State Filters */}
              <div className="bg-white rounded-2xl p-4 md:p-6 border border-slate-100 shadow-sm space-y-4">
                <div className="flex flex-col sm:flex-row gap-3">
                  ...
                  <div className="flex-1 relative">
                    ...
                    <input
                      ...
                      className="w-full pl-10 pr-4 py-2.5 rounded-xl border border-slate-200 bg-slate-50 hover:bg-white focus:bg-white focus:ring-2 focus:ring-[#8a1538] focus:border-transparent outline-none transition text-sm text-slate-800"
                    />
                  </div>
                ...
                <div className="flex items-center space-x-1.5 overflow-x-auto pb-1 border-t border-slate-100 pt-3">
                  ...
                  {CONFEDERATIONS.map(conf => (
                    ...
                    >
                      {conf.name.split(" ")[0]} {/* Show abbreviation */}
                    </button>
                  ))}
                </div>
              </div>
              ...
              <div className="space-y-6">
                {CONFEDERATIONS
                  ...
                  .map(conf => {
                    // Check if there's at least one visible country in this confederation
                    ...
                    return (
                      <div key={conf.id} className="space-y-4">
                        <div className="flex items-center justify-between border-b border-slate-200 pb-2">
                          <h2 className="font-extrabold text-[#8a1538] text-base md:text-lg tracking-tight uppercase">
                            {conf.name}
                          </h2>
                          <span className="text-xs font-semibold text-slate-400 bg-slate-100 px-2.5 py-1 rounded-full">
                            {visibleCountries.length} țări vizibile
                          </span>
                        </div>
                          ...
                          return (
                            <div key={country.code} className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden transition duration-200 hover:shadow-md">
                              ...
                              <div 
                                ...
                                className="flex items-center justify-between p-4 md:p-5 bg-[#fdfbf7] border-b border-slate-50 cursor-pointer select-none no-select"
                              >
                                <div className="flex items-center space-x-3">
                                  ...
                                  <div>
                                    <div className="flex items-center space-x-2">
                                      ...
                                      <span className="text-xs font-black text-[#b49141] bg-[#b49141]/5 border border-[#b49141]/10 px-1.5 py-0.5 rounded uppercase">
                                        {country.code}
                                      </span>
                                    </div>
                                ...
                                <div className="flex items-center space-x-3" onClick={(e) => e.stopPropagation()}>
                                  {/* Bulk Actions Dropdown or Quick Actions */}
                              ...
                              {/* Sticker Grid Items (Only visible if expanded) */}
                              {!isCollapsed && (
                                    ...
                                    {Array.from({ length: country.stickersCount }, (_, index) => {
                                      ...
                                      return (
                                        ...
                                        >
                                          ...
                                          <span className={`text-[10px] font-black uppercase ${isOwned ? "text-[#b49141]" : "text-slate-300"}`}>
                                            {country.code}
                                          </span>
                                          ...
                                          {/* Duplicate Badge & Adjusters */}
                                          {isOwned && (
                                            <>
                                              {/* Duplicate Multiplier indicator */}
                                              {isDuplicate && (
                                                <span className="absolute bottom-1 right-1 bg-[#b49141] text-slate-900 text-[9px] px-1 font-black rounded-sm shadow-sm scale-95">
                                                  x{count}
                                                </span>
                                              )}

                                              {/* Hover Overlay with plus/minus triggers */}
          ...
          {/* TAB 2: SWAP HUB (DUPLICATES & TRADING) */}
          {activeTab === "swap" && (
            <div className="bg-white rounded-2xl p-6 md:p-8 border border-slate-100 shadow-sm space-y-6">
              <div>
                ...
                <p className="text-sm text-slate-400 mt-1">
                  Aici poți genera o listă completă de stickere pe care le cauți sau pe care le ai la schimb pentru a le trimite rapid prietenilor tăi pe chat.
                </p>
              </div>

              <div className="bg-[#f7f5ee] rounded-xl p-4 flex items-start space-x-3 border border-slate-200/50">
                <Info className="w-5 h-5 text-[#8a1538] flex-shrink-0 mt-0.5" />
                <div className="text-xs text-slate-600 space-y-1">
                  <p className="font-bold text-slate-700">Cum se folosește?</p>
                  <p>Apasă pe butonul de mai jos pentru a copia textul formatat în clipboard. Trimite-l pe WhatsApp, Messenger, Discord sau Forum-uri de colecționari.</p>
                </div>
              </div>

              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  <h3 className="text-xs font-bold text-slate-400 uppercase tracking-wider">Previzualizare Listă Schimb</h3>
                  <button
                    onClick={copySwapListToClipboard}
                    className="flex items-center space-x-2 px-4 py-2 bg-[#8a1538] hover:bg-[#72102e] text-white text-xs font-bold rounded-xl shadow-md transition"
                  >
                    {swapCopied ? (
                      <>
                        <Check className="w-4 h-4 text-[#b49141]" />
                        <span>Copiat cu succes!</span>
                      </>
                    ) : (
                      <>
                        <Copy className="w-4 h-4 text-[#b49141]" />
                ...
                <div className="bg-slate-900 text-[#f7f5ee] p-5 rounded-2xl font-mono text-xs overflow-x-auto whitespace-pre-wrap leading-relaxed max-h-[400px] border border-slate-800 shadow-inner">
                  <span className="text-[#b49141]">📋 LISTĂ SCHIMB PANINI 2026 (Cod Album: {id.toUpperCase()})</span>
                  {"\n\n"}
          ...
          {activeTab === "settings" && (
            <div className="bg-white rounded-2xl p-6 md:p-8 border border-slate-100 shadow-sm space-y-6">
              <div>
                ...
                <p className="text-sm text-slate-400 mt-1">
                  Administrează integritatea datelor tale, salvează manual backup-uri sau curăță albumul pentru un nou început.
                </p>
              </div>
                ...
                <div className="border border-slate-200/60 p-5 rounded-2xl flex flex-col justify-between space-y-4">
                  <div>
                    <h3 className="font-bold text-slate-700 text-sm flex items-center space-x-2">
                      <FileDown className="w-4 h-4 text-[#b49141]" />
                      <span>Exportă Fișier Backup</span>
                    </h3>
                    <p className="text-xs text-slate-400 mt-1">
                      Descarcă un fișier JSON cu toate datele tale. Recomandat periodic pentru securizarea colecției tale.
                    </p>
                  </div>
                  <button
              ...
              <div className="border border-rose-100 bg-rose-50/20 p-5 rounded-2xl space-y-4">
                <div>
                  ...
                  <p className="text-xs text-rose-500 mt-1">
                    Resetarea albumului va șterge toate datele deținute de pe acest album. Această acțiune este ireversibilă în baza de date.
                  </p>
                </div>
                <button
